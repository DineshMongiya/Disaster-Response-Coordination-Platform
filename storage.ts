...
export class DatabaseStorage implements IStorage {
  ...
  async getDisasters(filters?: { tag?: string; ownerId?: string }): Promise<Disaster[]> {
    ...
    if (filters?.tag) {
      conditions.push(sql`${disasters.tags} && ARRAY[${filters.tag}]`);
    }
    
    if (filters?.ownerId) {
      conditions.push(eq(disasters.ownerId, filters.ownerId));
    }
    
    if (conditions.length > 0) {
      return await db.select().from(disasters).where(and(...conditions));
    }
